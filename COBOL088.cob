      ...
      *----                                                             00197000
       01  CAB01.                                                       00198000
           03 FILLER                       PIC X(019) VALUE 'COBOL088'. 00199000
      ...
      *----                                                             00292600
       01  TRAILLER.                                                    00292700
           03 FILLER                       PIC X(040) VALUE             00292800
              'TOTAL DA FOLHA DE PAGAMENTO'.
           03 FILLER                       PIC X(004) VALUE SPACES.     00292900
           03 FILLER                       PIC X(010) VALUE SPACES.     00293000
           03 FILLER                       PIC X(006) VALUE SPACES.     00293100
           03 SALARIO-TOT                  PIC X(012) VALUE SPACES.     00293200
           03 FILLER                       PIC X(006) VALUE SPACES.     00293300
           03 FILLER                       PIC X(005) VALUE SPACES.     00293400
      *-------------------                                              00293500
      ...
      *----                                                             00295000
       ...
       1001-LER-FUNCI.                                                  00359000
           ...
           IF ST-FUNCI NOT EQUAL 0 AND WSS-EOF NOT = 'Y'  THEN          00354000
              DISPLAY 'COBOL088 - ERRO READ FUNCI  FS=' ST-FUNCI        00356000
           ...
           PERFORM UNTIL WSS-EOF = 'Y'
              ...
              IF   CNT-LINHAS GREATER 60                                   00440
              ...
              IF ST-RLATFUN NOT EQUAL 0 THEN                            00354000
                DISPLAY 'COBOL088 - ERRO WRITE FOLHAFUN FS='            00356000
                ST-RLATFUN
              ...
              IF ST-FUNCI NOT EQUAL 0 AND WSS-EOF NOT = 'Y'  THEN
                 DISPLAY 'COBOL088 - ERRO READ FUNCI FS=' ST-FUNCI
                 MOVE ST-FUNCI    TO RETURN-CODE
                 PERFORM ROT-ABEND
                    THRU ROT-ABEND-EXIT
              END-IF
      ...
      *----                                                             00520400
       2000-CALCULA-SALARIO.                                            00520500
           ...
           ADD SALARIO-EXT  TO GERAL-SALARIO.
      ...
      *----                                                             00521000
       3000-IMPRIME-CABECALHOS.                                         00522000
           ...
           ADD   4           TO WSS-REG-GRAVADOS

           MOVE  5           TO CNT-LINHAS                              00524000

           PERFORM GRAVA-RELATORIO.

       ROT-IMPRIME-TRAILLER.


           MOVE GERAL-SALARIO   TO MASCARA-SALARIO.                     00520700
           MOVE MASCARA-SALARIO TO SALARIO-TOT.                            00410

           PERFORM GRAVA-TRAILLER.
      ...
      *----                                                             00351000
       ...
       1000-OPEN-FUNCI.                                                 00352000
           ...
           IF ST-FUNCI NOT EQUAL 0 THEN                                 00354000
              DISPLAY 'COBOL088 - ERRO OPEN FUNCI FS=' ST-FUNCI         00356000
              MOVE ST-FUNCI                  TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF.
           ...
           IF ST-RLATFUN NOT EQUAL 0 THEN                               00354000
              DISPLAY 'COBOL088 - ERRO OPEN FOLHAFUN FS=' ST-RLATFUN    00356000
              MOVE ST-RLATFUN                TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF.
      ...
      *----                                                             00510000
       GRAVA-RELATORIO.
           ...
           IF ST-RLATFUN NOT EQUAL 0 THEN                               00354000
              DISPLAY 'COBOL088 - ERRO WRITE FOLHAFUN FS=' ST-RLATFUN   00356000
              MOVE ST-RLATFUN  TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF
       ...
       GRAVA-TRAILLER.
           ...
           WRITE REG-RLATFUN FROM TRAILLER AFTER ADVANCING 3               00470
           END-WRITE.

           IF ST-RLATFUN NOT EQUAL 0 THEN                               00354000
              DISPLAY 'COBOL088 - ERRO WRITE TRAILLER FS=' ST-RLATFUN   00356000
              MOVE ST-RLATFUN  TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF.
       ...
       1002-FECHAR-FUNCI.                                               00520000
           ...
           IF ST-FUNCI NOT EQUAL 0 THEN
              DISPLAY 'COBOL088 - ERRO CLOSE FUNCI FS=' ST-FUNCI
              MOVE ST-FUNCI    TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF.
           ...
           IF ST-RLATFUN NOT EQUAL 0 THEN
              DISPLAY 'COBOL088 - ERRO CLOSE RELATFUN FS=' ST-RLATFUN
              MOVE ST-RLATFUN  TO RETURN-CODE
              PERFORM ROT-ABEND
                 THRU ROT-ABEND-EXIT
           END-IF.
